use tokio::task;
use unsync::fixed_broadcast;

const END: u32 = 16;

#[tokio::test]
async fn test_fixed_broadcast() -> Result<(), Box<dyn std::error::Error>> {
    let local = task::LocalSet::new();

    let mut tx = fixed_broadcast::channel::<16, _>();
    ...
    let (receivers, b) = local
        .run_until(async move {
            ...
            for _ in 0..16 {
                let mut rx = tx.subscribe().unwrap();
...
#[tokio::test]
async fn test_fixed_broadcast_receiver_drop() {
    let mut tx = fixed_broadcast::channel::<16, _>();
    let mut sub1 = tx.subscribe().unwrap();
    let mut sub2 = tx.subscribe().unwrap();
